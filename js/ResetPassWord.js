...
window.onload = function () {
  ...
  var a;
  document.getElementById("reset").addEventListener("submit", function (b) {
    b.preventDefault();
    b = document.getElementById("wsurl").value;
    var f = document.getElementById("2fa").value;
    var d = document.getElementById("password").value;
    try {
      a = new WebSocket(b);
    } catch (a) {
      c("Invalid websocket address!");
    }
    a.onerror = function (a) {
      c("Connection error to the websocket server.");
    };
    a.onmessage = function (b) {
      return h.asyncExecutePromiseGeneratorProgram(function (e) {
        if (e.nextAddress == 1) {
          b = JSON.parse(b.data);
          if (b.Status == "Username&PassWord") {
            c("Sending information to the server!");
            return e.return(a.send(JSON.stringify({
              type: "resetPassword",
              Otp: f,
              NewPassword: d
            })));
          ...
          c("Password changed successfully! Redirecting in 3 seconds!");
        ...
        if (b.Status == 0) {
          c("You have entered an incorrect OTP code or have been blocked after three failed attempts. Please try again after restarting the bot.");
        }
        e.jumpToEnd();
      });
    };
